...
export function AboutMe() {
    ...
    return(
        <main id="aboutme" className="max-w-7xl m-auto px-8 py-24">
            <h2 className="text-4xl font-alt mb-8">Sobre Mim</h2>
            <p className="text-lg font-normal xl:text-xl">
                    ...
                    <div className='grid gap-4 grid-cols-3 md:grid-cols-5'>
                    {
                        hardSkillsInfo.map((skill, index) => (
                            <div key={index} className='flex flex-col items-center'>
                                <Image src={skill.imagePath} alt={skill.name} width={64} height={64} />
                                <h5 className='mt-4 text-lg'>{skill.name}</h5>
                            </div>
                        ))
                    }
                    </div>
